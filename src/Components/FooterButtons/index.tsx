...
const { changeStyle} = useContext(AlpacaContext);
  ...
  const GetRandomItem = () => {
    ...
    const RandomKeyMouth = keysMouth[Math.floor(Math.random() * keysMouth.length)];
    ...
    const RandomKeyAcessories = keysAcessories[Math.floor(Math.random() * keysAcessories.length)];
    changeStyle(RandomKeyAcessories, "Acessories");

    const keysBackground = Object.keys(Dicionario.Background);
    const RandomKeyBackground = keysBackground[Math.floor(Math.random() * keysBackground.length)];
